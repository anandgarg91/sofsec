...
class plant:
	def __init__(self,maxstep):
		...
		init=[0.0,0.0,490,490,200.00,200.00,200.00]
		self.result=[[0 for x in range(7)] for x in range(maxstep+1)]
		self.result[0] = init

	def Actuator(self, P1, P2, P3, P4, P5, P6): # Viewer should be notified that passing the output value from plc directly to the input value(return value) is not quite true. For actuators (in here, we are talking about motor valve, pump, pressure pump and ultra violet), the behavior is unkown, it could take seconds for the actuators to process the insctruction, meaning there's a lapse between the instruction sent and carried out.                --PF
