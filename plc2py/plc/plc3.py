...
# there could be CIP in the future, but current SWaT system has none
	def __init__(self,HMI):
		self.TON_FIT301_P1_TM = TONR(6)
		self.TON_FIT301_P2_TM = TONR(6)
		self.SEC_TEST = 0
		self.MIN_TEST = 0
		self.Mid_NEXT = 0
		...
		self.LIT301_FB = AIN_FBD(0, 31208, 3277, 16383, 1250, 0.0, HMI.LIT301)
		self.P_UF_FEED_DUTY_FB = Duty2_FBD()
		self.P301_FB = PMP_FBD(3,3,HMI.P301)
		self.P302_FB = PMP_FBD(3,3,HMI.P302)
		self.FIT301_FB = FIT_FBD(-15, 31208, 3277, 16383, 4, 0.0,HMI.FIT301)
		self.PSH301_FB = SWITCH_FBD(HMI.PSH301)
		self.DPSH301_FB = SWITCH_FBD(HMI.DPSH301)
		self.DPIT301_FB = AIN_FBD(-30, 31208, 3277, 16383, 100, 0.0, HMI.DPIT301)
		self.MV301_FB = MV_FBD(10,10,HMI.MV301)
		self.MV302_FB = MV_FBD(10,10,HMI.MV302)
		self.MV303_FB = MV_FBD(10,10,HMI.MV303)
		self.MV304_FB = MV_FBD(10,10,HMI.MV304)

		print "	PLC3 Started\n"

	def Pre_Main_UF_Feed(self,IO_DI_WIFI,IO,HMI,Sec_P,Min_P,Hrs_P):
...
#	def Main_Seq(self,Sec_P,Min_P):
		...
		#P6self.Mid_P602_AutoInp.On = self.Mid_P602_AutoInp
...
#this R_TRIG_BW seems only needed when chemical backwash is needed
		...
		while switch(HMI.P3.State):
			...
			if case(3):
				...
				HMI.Cy_P3.CIP_CLEANING_SEC	=0
				HMI.Cy_P3.DRAIN_SEC			=0
				if  HMI.P_UF_FEED_DUTY.Pump_Running or self.Mid_NEXT:
					self.Mid_NEXT =0
					HMI.P3.State=4
				break
			if case(4):
			...
			if case(8):
				...
				HMI.Cy_P3.CIP_CLEANING_SEC	=0
				HMI.Cy_P3.DRAIN_SEC			=0
				if not HMI.P_UF_FEED_DUTY.Pump_Running or self.Mid_NEXT:
					self.Mid_NEXT =0
					HMI.P3.State=9				
				break
			if case(9):
			...
			if case(17):     
				...
				HMI.Cy_P3.CIP_CLEANING_SEC	=0
				HMI.Cy_P3.DRAIN_SEC			=0
				if HMI.P_NAOCL_UF_DUTY.Pump_Running or self.Mid_NEXT:
					self.Mid_NEXT=0
					HMI.P3.State=18			
				break
			if case(18):      
			...
			if case(19):    
				...
				HMI.Cy_P3.BACKWASH_SEC		=0
				HMI.Cy_P3.DRAIN_SEC			=0
				if not HMI.P_NAOCL_UF_DUTY.Pump_Running or self.Mid_NEXT:
						self.Mid_NEXT=0
						HMI.P3.State=14
				break
			if case(99): 
...
#	def UF_Feed(self):
		self.LIT301_FB.AIN_FBD(IO_DI_WIFI.RIO3, IO.LIT301, HMI.LIT301) 
		self.P_UF_FEED_DUTY_FB.Duty2_FBD(self.Mid_P_UF_FEED_DUTY_AutoInp, HMI.P301, HMI.P302, HMI.P_UF_FEED_DUTY)
		self.P301_FB.PMP_FBD(self.P_UF_FEED_DUTY_FB.Start_Pmp1, IO.P301, HMI.P301, Min_P)
		self.P302_FB.PMP_FBD(self.P_UF_FEED_DUTY_FB.Start_Pmp2, IO.P302, HMI.P302, Min_P)
		self.FIT301_FB.FIT_FBD(IO_DI_WIFI.RIO3, self.Mid_FIT301_Tot_Enb, IO.FIT301, HMI.FIT301, Sec_P)
		self.PSH301_FB.SWITCH_FBD(IO.PSH301, HMI.PSH301)
		self.DPSH301_FB.SWITCH_FBD(IO.DPSH301, HMI.DPSH301)
		self.DPIT301_FB.AIN_FBD(IO_DI_WIFI.RIO3, IO.DPIT301, HMI.DPIT301)
