...
class AIN_FBD:
	...
	def __init__(self,H_Raw_WRIO, L_Raw_WRIO, H_Raw_RIO, L_Raw_RIO,HEU,LEU,HMI):
		...
		if Remote:
			if not Auto:
				...
				if HMI.Cmd == 2:
					if not self.Fault and not self.FT_Start and not self.FT_Stop and Permissive == -1 and bit_2_signed_integer(HMI.SD) == 0:
						self.Cmd_Start = 1
					elif self.Cmd_Start and bit_2_signed_integer(HMI.SD) != 0 or Fault:
						self.SD = bit_2_signed_integer(HMI.SD)
						self.Cmd_Start = 0
						HMI.Cmd = 1
					if TON_Start.DN and not Run:
						self.FT_Start = 1
						self.FT_Stop  = 0
						self.Cmd_Start = 0
						HMI.Cmd  = 1
				...
				if Run:
					self.Cmd = 2
				if not Run or Fault or self.FT_Start or self.Stop:
					self.Cmd = 1
				if AutoInp:
					if not self.Fault and not self.FT_Start and not self.FT_Stop and Permissive == -1 and bit_2_signed_integer(HMI.SD) == 0:
						self.Cmd_Start = 1
					elif self.Cmd_Start or bit_2_signed_integer(HMI.SD) != 70 or Fault:
						self.SD = bit_2_signed_integer(HMI.SD)
						self.Cmd_Start = 0
					if self.TON_Start.DN and not Run:
						self.FT_Start = 1
		...
		if Remote:
			if not Auto:
				if HMI.Cmd == 1:
					...
					if TON_Stop.DN and VSD_In.Active:
						self.FT_Start = 0
						self.FT_Stop  = 1
				if HMI.Cmd == 2:
